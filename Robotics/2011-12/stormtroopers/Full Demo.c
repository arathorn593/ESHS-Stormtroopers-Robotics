...
#pragma config(Motor,  port6,           liftRight,     tmotorNormal, openLoop)
#pragma config(Motor,  port7,           liftLeft,      tmotorNormal, openLoop, reversed)
...
task main{
  ...
  while(1 == 1){
    ...
    else                      //NEW SECTION THIS IS PARTNER CONTROLLED
    {
    ...
    if( abs(vexRT[Ch3Xmtr2]) > 50){         //threshold for right side
    ...
    if((vexRT[Btn8LXmtr2]) == 1)  //runs center omni wheels for crabing
      {
      motor[middleFront] = -127;
      motor[middleBack]= -127;
      }
